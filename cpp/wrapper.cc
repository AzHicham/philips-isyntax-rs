...
ViewWrapper open_file(const std::unique_ptr<PixelEngine>& pixel_engine, const std::string& filename) {
...
uint32_t level_count(const ViewWrapper& view_wrapper) { return view_wrapper.view->numDerivedLevels() + 1; }
...
rust::Vec<Range> envelopes_range(const ViewWrapper& view_wrapper, uint32_t level) {
    ...
    for (auto& range : envelopes_range) {
        res.push_back(Range{range[0], range[1], range[2], range[3]});
    }
    return res;
}

void read_region(const std::unique_ptr<PixelEngine>& pixel_engine, const ViewWrapper& view_wrapper,
                 const RegionRequest& request, rust::Vec<uint8_t>& buffer, Size& image_size) {
