...
std::shared_ptr<Region> request_region(View& view, const Rectangle &range, bool async_, const std::array<size_t, 3>& bg_color) {
...
void get(std::shared_ptr<Region> region, rust::Vec<uint8_t>& buffer) {
    region->get(buffer.data(), buffer.size());
}
